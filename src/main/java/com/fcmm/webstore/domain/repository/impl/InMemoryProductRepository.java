...
public class InMemoryProductRepository implements ProductRepository {
	...
	public Product getProductById(String pProductId) {
		...
		for (Product product : listOfProducts) {
			if (product != null && product.getProductId() != null && product.getProductId().equals(pProductId)) {
				productById = product;
				break;
			}
		}
		...
		if (productById == null) {
			throw new ProductNotFoundException("No products found with the product id: " + pProductId);
		}
	...
	public List<Product> getProductsByCategory(String pCategory) {
		...
		for (Product product : listOfProducts) {
			if (pCategory.equalsIgnoreCase(product.getCategory())) {
	...
	public Set<Product> getProductsByFilter(Map<String, List<String>> pFilterParams) {
		...
		Set<String> criteria = pFilterParams.keySet();

		if (criteria.contains("brand")) {
			for (String brandName : pFilterParams.get("brand")) {
				for (Product product : listOfProducts) {
		...
		if (criteria.contains("category")) {
			for (String category : pFilterParams.get("category")) {
